...
async function testUserOperations() {
  ...
  // Test user creation with proper UUID and correct schema
  const testUser = {
    ...
    role: 'employee',
    password: 'test123',
    join_date: new Date().toISOString().split('T')[0], // Add required join_date field
    is_active: true, // Add is_active field
    created_at: new Date().toISOString()
    // Note: removed updated_at as it may not exist in the schema
  };
  ...
  try {
    ...
    if (!createError) {
      ...
      // Update user (only update fields that exist)
...
async function testClientOperations() {
  ...
  const testClient = {
    ...
    email: `client-${Date.now()}@testclient.com`, // Unique email
    phone: '123-456-7890',
    team: 'creative',
...
async function testTaskOperations() {
  ...
  const testClient = {
    ...
    email: `taskclient-${Date.now()}@example.com`,
    phone: '123-456-7890',
    team: 'creative',
    date_added: new Date().toISOString().split('T')[0],
    created_at: new Date().toISOString()
...
async function testDailyWorkEntryOperations() {
  ...
  const testUser = {
    id: uuidv4(),
    name: 'DWE Test User',
    email: `dweuser-${Date.now()}@example.com`,
    team: 'creative',
    role: 'employee',
    password: 'test123',
    join_date: new Date().toISOString().split('T')[0], // Add required join_date field
...
async function testDataIntegrity() {
  ...
  try {
    ...
    const invalidTask = {
      ...
      status: 'todo',
      priority: 'medium',
      assignee_id: uuidv4(), // Non-existent user
      client_id: uuidv4(), // Non-existent client
      team: 'creative',
      due_date: new Date().toISOString().split('T')[0],
      created_by: uuidv4(), // Non-existent user
      created_at: new Date().toISOString()
    };
    ...
    const { error: duplicateError } = await supabase
      .from('users')
      .insert([{ ...duplicateUser, id: uuidv4() }]);
