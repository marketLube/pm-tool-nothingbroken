...
const WebTeam: React.FC = () => {
  ...
  const notStartedTasks = teamTasks.filter(task => 
    task.status === 'not_started' || 
    task.status === 'proposal_awaiting'
  ).length;
  ...
  const getFilteredTasks = () => {
    let filtered = teamTasks.filter(task => task.status !== 'completed');
              ...
              <ul className="divide-y divide-gray-100">
                {tasksByClient.map(({ client, clientId, tasks, activeTasks, completedTasks }) => (
                  <li 
                    key={client?.id} 
                    className={`p-4 hover:bg-gray-50 transition-colors cursor-pointer ${
                      selectedClient === clientId ? 'bg-blue-50' : ''
                    }`}
                    onClick={() => handleClientClick(clientId)}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 truncate hover:text-blue-700">{client?.name}</p>
