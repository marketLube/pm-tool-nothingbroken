...
// Modal component for employee tasks
...
const EmployeeTasksModal: React.FC<EmployeeTasksModalProps> = ({ isOpen, onClose, employee, tasks }) => {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            <Avatar src={employee.avatar} name={employee.name} size="md" />
            <div>
              <h3 className="text-lg font-bold text-gray-900">{employee.name}'s Tasks</h3>
        ...
        <div className="p-4 overflow-y-auto flex-1">
          {tasks.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="mt-4 text-gray-500">No tasks assigned to this employee</p>
            </div>
          )}
        </div>
...
const WebTeam: React.FC = () => {
  ...
  const [employeeTasksModalOpen, setEmployeeTasksModalOpen] = useState(false);
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      try {
        ...
        console.log(`[WebTeam Database Search] Found ${searchResults.length} tasks`);
      } catch (error) {
  ...
  const sortedReports = useMemo(() => {
    return [...getMemberRecentReports(selectedEmployee?.id || '')].sort((a, b) => {
      const dateA = new Date(b.date);
      const dateB = new Date(a.date);
      return dateA.getTime() - dateB.getTime();
    });
  ...
  // Add scroll container ref for horizontal scrolling
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
        ...
        >
          <Button
            ...
            className="shadow-md hover:shadow-lg bg-gradient-to-r from-blue-600 to-cyan-600 text-white hover:from-blue-700 hover:to-cyan-700 transform hover:scale-105 transition-all duration-300"
          >
            <span className="text-sm font-medium text-white">New Web Task</span>
          </Button>
        </PermissionGuard>
      </div>
                    ...
                    <CardContent className="p-4 flex flex-col h-full">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          ...
                          ) : status.id === 'not_started' || status.id === 'proposal_awaiting' ? (
                  ...
                  return (
                    <li 
                      ...
                      className={`p-4 hover:bg-gray-50 transition-colors cursor-pointer ${
                        selectedEmployee?.id === member.id ? 'bg-blue-50' : ''
                      }`}
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 truncate hover:text-blue-700">{member.name}</p>
                          <p className="text-xs text-gray-500 truncate">{member.role === 'admin' ? 'Admin' : member.role === 'manager' ? 'Team Lead' : 'Team Member'}</p>
                        </div>
                        <div className="inline-flex items-center bg-blue-50 px-2.5 py-0.5 rounded-full text-xs font-medium text-blue-800">
                          {activeTasks} Active
                        </div>
                      </div>
