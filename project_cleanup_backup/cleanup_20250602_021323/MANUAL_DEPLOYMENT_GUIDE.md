...
## **STEP-BY-STEP SUPABASE DASHBOARD DEPLOYMENT**

### **Step 1: Navigate to Functions**
1. Open: https://supabase.com/dashboard/project/ysfknpujqivkudhnhezx/functions
2. You should see the "Edge Functions" page

### **Step 2: Create New Function**
1. Click **"Create a new function"** button (usually green/blue button)
2. Enter function name: `scheduled-rollover` (exactly as shown)
3. Click **"Create function"** or **"Next"**

### **Step 3: Copy Function Code**
Copy the ENTIRE content below and paste it into the editor:
...
function getIndiaDateTime(): Date {
  ...
  const ist = new Date(utc + ((IST_OFFSET_HOURS * 60 + IST_OFFSET_MINUTES) * 60000));
  return ist;
}
...
1. Click **"Deploy function"** or **"Save and Deploy"**
2. Wait for deployment to complete (should show green success message)
3. You should see the function listed as "Active" or "Deployed"

### **Step 5: Verify Deployment**
1. Copy the function URL (should be something like):
   `https://ysfknpujqivkudhnhezx.supabase.co/functions/v1/scheduled-rollover`
2. Test it using curl or browser
...
1. **Function name mismatch**: Make sure it's exactly `scheduled-rollover`
2. **Code formatting**: Make sure you copied the ENTIRE code block
3. **Deployment timeout**: Try refreshing the page and check if function appears
4. **Permissions**: Make sure you have admin access to the Supabase project

### **Alternative: CLI Deployment (if manual fails)**
If manual deployment keeps failing, we can try the CLI approach with Docker.
...
1. Function listed in the dashboard as "Active"
2. Function URL responds without "NOT_FOUND" error
3. Test curl returns proper JSON response 